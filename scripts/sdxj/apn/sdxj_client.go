...
type DeviceInfo struct {
	DeviceID         string `json:"id"`        // 设备通道ID
	ChannelID        string `json:"channel"`   // 通道号
	...
	MaintainUnitName string `json:"maintain_Unitname"`
	MaintainUnitID   string `json:"maintain_Unit"`
...
type TerminalInfoResp struct {
	Successful  bool               `json:"successful"`
	ResultValue TerminalInfoResult `json:"resultValue"`
}
...
type TerminalInfo struct {
	DeviceID    string `json:"id"`           // 终端设备ID
	LineName    string `json:"line_Name"`    // 线路名称
	TowerName   string `json:"tower_Name"`   // 杆塔名称
	DeviceState string `json:"device_State"` // 设备状态 0：离线 1：在线
	UpdateTime  int64  `json:"update_Time"`  // 最新同步时间
...
// GetTerminalInfo 通过设备编码获取设备信息
func (s *SDXJClient) GetTerminalInfo(code string) (TerminalInfo, error) {
	token, err := s.GetToken()
	if err != nil {
		return TerminalInfo{}, err
	}

	res := TerminalInfoResp{}
	...
	resp, err := s.client.R().SetAuthToken(token).SetQueryString(fmt.Sprintf("params=%s", paramsStr)).SetResult(&res).Get("tTerminalInfo/")
	if err != nil {
		// 判断错误类型
		switch e := err.(type) {
		case *resty.ResponseError:
			// 如果是响应错误，说明服务器返回了错误响应
			logrus.Errorf("SDXJClient GetTerminalInfo Response error: %s", e)

			// 获取响应体
			if e.Response != nil {
				logrus.Errorf("SDXJClient GetTerminalInfo Response Error body:%s", string(e.Response.Body()))
			}
	...
	logrus.Debug("SDXJClient GetTerminalInfo success resp: ", resp.String())

	if !res.Successful {
		logrus.Errorf("SDXJClient GetDeviceInfoList failed resp: %s", resp.String())
		return TerminalInfo{}, errors.New("SDXJClient GetTerminalInfo failed")
	}

	if len(res.ResultValue.Items) == 0 {
		logrus.Errorf("SDXJClient GetTerminalInfo Items Empty: %s", resp.String())
		return TerminalInfo{}, errors.New("SDXJClient GetTerminalInfo Items Empty")
	}

	return res.ResultValue.Items[0], nil
}
